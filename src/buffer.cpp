...
std::unique_ptr<VertexBuffer> VertexBuffer::create(std::vector<Vertex> vertices, DeviceManager *deviceManager,
...
void VertexBuffer::init(std::vector<Vertex> vertices, DeviceManager *deviceManager, VkCommandPool commandPool)
{
	...
	// 		VK_MEMORY_PROPERTY_HOST_COHERENT_BIT : CPU에서 GPU 메모리의 값을 수정하면 그 즉시 GPU 메모리와 캐시에 해당
	// 값을 수정하는 설정
	//      									  (원래는 CPU에서 GPU 메모리 값을 수정하면 GPU 캐시를 플러쉬하여 다시
	//      캐시에 값을 올리는 형식으로 동작)
	...
	vkMapMemory(device, stagingBufferMemory, 0, bufferSize, 0,
				&data); // GPU 메모리에 매핑된 CPU 메모리 포인터 반환 (현재는 버텍스 버퍼 메모리 전부를 매핑)
	memcpy(data, vertices.data(),
		   (size_t)bufferSize); // CPU 메모리 포인터는 가상포인터로 data에 정점 정보를 복사하면 GPU에 즉시 반영
								// 실제 CPU 메모리에 저장되는게 아닌 CPU 메모리 포인터는 가상 포인터역할만 하고 GPU
								// 메모리에 바로 저장 메모리 유형의 속성에 의해 GPU 캐시를 플러쉬할 필요없이 즉시 적용
	...
	// 		VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT : GPU 전용 메모리에 데이터를 저장하여, GPU가 최적화된 방식으로 접근할 수
	// 있게 함.
	createBuffer(physicalDevice, bufferSize, VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_VERTEX_BUFFER_BIT,
				 VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, buffers[0], buffersMemory[0]);
...
std::unique_ptr<IndexBuffer> IndexBuffer::create(std::vector<uint32_t> indices, DeviceManager *deviceManager,
												 VkCommandPool commandPool)
{
...
void IndexBuffer::init(std::vector<uint32_t> indices, DeviceManager *deviceManager, VkCommandPool commandPool)
{
	...
	// 		VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT : GPU 전용 메모리에 데이터를 저장하여, GPU가 최적화된 방식으로 접근할 수
	// 있게 함.
	createBuffer(physicalDevice, bufferSize, VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_INDEX_BUFFER_BIT,
				 VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, buffers[0], buffersMemory[0]);
...
void StagingBuffer::init(DeviceManager *deviceManager, VkDeviceSize imageSize)
{
	...
	buffers.resize(size);
	buffersMemory.resize(size);
	createBuffer(deviceManager->getPhysicalDevice(), imageSize, VK_BUFFER_USAGE_TRANSFER_SRC_BIT,
				 VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, buffers[0],
				 buffersMemory[0]);
}
...
*/
void Buffer::createBuffer(VkPhysicalDevice physicalDevice, VkDeviceSize size, VkBufferUsageFlags usage,
...
void Buffer::copyBuffer(VkQueue graphicsQueue, VkCommandPool commandPool, VkBuffer srcBuffer, VkBuffer dstBuffer,
						VkDeviceSize size)
{
