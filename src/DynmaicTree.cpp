...
void DynamicTree::InsertLeaf(int32_t leaf)
{
	...
	while (nodes[index].IsLeaf() == false)
	{
		...
		{
			cost1 = GetInsertionCostForLeaf(leafAABB, child1);
		}
		else
		{
			cost1 = GetInsertionCost(leafAABB, child1);
		}

		float cost2;
		if (nodes[child2].IsLeaf())
		{
			cost2 = GetInsertionCostForLeaf(leafAABB, child2);
		}
		else
		{
			cost2 = GetInsertionCost(leafAABB, child2);
		}
...
float DynamicTree::GetInsertionCostForLeaf(const AABB &leafAABB, int32_t child)
{
	AABB aabb;
...
float DynamicTree::GetInsertionCost(const AABB &leafAABB, int32_t child)
{
	AABB aabb;
	aabb.Combine(leafAABB, nodes[child].aabb);
