...
class Buffer
{
  ...
  protected:
	...
	void copyBuffer(VkQueue graphicsQueue, VkCommandPool commandPool, VkBuffer srcBuffer, VkBuffer dstBuffer,
					VkDeviceSize size);
...
{
  public:
	static std::unique_ptr<VertexBuffer> create(std::vector<Vertex> vertices, DeviceManager *deviceManager,
...
{
  public:
	static std::unique_ptr<IndexBuffer> create(std::vector<uint32_t> indices, DeviceManager *deviceManager,
											   VkCommandPool commandPool);
