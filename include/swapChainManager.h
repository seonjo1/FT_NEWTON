...
{
  public:
	static std::unique_ptr<SwapChainManager> create(GLFWwindow *window, DeviceManager *deviceManager,
													VkSurfaceKHR surface);
	~SwapChainManager() = default;
	void createFramebuffers(DeviceManager *deviceManager, VkRenderPass renderPass);
	void recreateSwapChain(GLFWwindow *window, DeviceManager *deviceManager, VkSurfaceKHR surface,
  ...
  private:
	...
	void createImage(VkPhysicalDevice physicalDevice, uint32_t width, uint32_t height, uint32_t mipLevels,
